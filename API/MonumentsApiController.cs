...
namespace OtvorenoRacunalstvoLabosi.API
{
    ...
    public class MonumentsApiController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Monument monumentDto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] Monument monument)
        {
            if (!ModelState.IsValid)
                return BadRequest(new { success = false, message = "Invalid data" });

            var updated = await _monumentService.Update(id, monument);
            if (!updated)
                return NotFound(new { success = false, message = "Monument not found" });
